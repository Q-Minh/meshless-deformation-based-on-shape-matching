...
struct simulation_params_t
{
	...
	mti855::physics::deformation_type_t deformation_type = mti855::physics::deformation_type_t::linear;
...
int main(int argc, char * argv[])
{
	...
	mti855::physics::deformable_mesh object;
	object.load_from_obj("../data/bunny.obj");
	...
	menu.callback_draw_custom_window = [&]()
	{
		...
		if (ImGui::CollapsingHeader("Data", ImGuiTreeNodeFlags_DefaultOpen))
		{
			...
			ImGui::Text("Bytes per vertex: %d", sim_params.total_memory / sim_params.vertices);
		}
		...
		{
			if (ImGui::Button("bunny", ImVec2(80, 20)))
			{
				object.load_from_obj("../data/bunny.obj");
				dirty = true;
			}

			if (ImGui::Button("teddy", ImVec2(80, 20)))
			{
				object.load_from_obj("../data/teddy.obj");
				dirty = true;
			}

			if (ImGui::Button("cube", ImVec2(80, 20)))
			{
				object.load_from_obj("../data/cube.obj");
				dirty = true;
			}
		}
	...
	pick_t pick;
	viewer.callback_mouse_down = [&pick, &object](igl::opengl::glfw::Viewer& viewer, int button, int modifier) -> bool
	{
	...
	viewer.callback_mouse_up = [&pick](igl::opengl::glfw::Viewer& viewer, int button, int modifier) -> bool
	{
		using button_type = igl::opengl::glfw::Viewer::MouseButton;
		if (static_cast<button_type>(button) != button_type::Left)
			return false;
	...
	viewer.callback_key_pressed = [&user_input_force](igl::opengl::glfw::Viewer& viewer, unsigned int key, int modifiers) -> bool
	{
		switch (key)
		{
		case 'i':
			user_input_force = user_input_force_t::up;
			break;
		case 'k':
			user_input_force = user_input_force_t::down;
			break;
		case 'j':
			user_input_force = user_input_force_t::left;
			break;
		case 'l':
			user_input_force = user_input_force_t::right;
			break;
		case 'b':
			user_input_force = user_input_force_t::backward;
			break;
		case GLFW_KEY_SPACE:
			user_input_force = user_input_force_t::forward;
			break;
		default:
			return false;
		}
	...
	auto const& simulate = [&]()
	{
		if (user_input_force != user_input_force_t::none)
		{
			...
			{
			case user_input_force_t::up:
				force4d(1) = 1.;
				break;
			case user_input_force_t::down:
				force4d(1) = -1.;
				break;
			case user_input_force_t::left:
				force4d(0) = -1.;
				break;
			case user_input_force_t::right:
				force4d(0) = 1.;
				break;
			case user_input_force_t::forward:
				force4d(2) = 1.;
				break;
			case user_input_force_t::backward:
				force4d(2) = -1.;
				break;
			}
			...
			Eigen::Matrix4d const screen_to_world_transform = (viewer.core().proj * viewer.core().view).inverse().cast<double>();
	...
	viewer.callback_pre_draw = [&new_physics_update, &simulate, &sim_params, &pick, &object, &start](igl::opengl::glfw::Viewer& viewer) -> bool
	{
		...
		if (pick.picked)
		{
			Eigen::Vector3d p1{ pick.object->V()(pick.vi, 0), pick.object->V()(pick.vi, 1), pick.object->V()(pick.vi, 2) };
			...
			Eigen::Vector3d const p2 = igl::unproject(Eigen::Vector3f(x2, y2, 1.0f), viewer.core().view, viewer.core().proj, viewer.core().viewport).cast<double>();
		...
		auto const period_in_nanoseconds = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
		start = end;
