...
namespace mti855 {
	namespace physics {
		...
		Eigen::MatrixX3d deformable_mesh::apply_force(Eigen::Vector3d const & location, Eigen::Vector3d const & force)
		{
			...
			for (unsigned int i = 0; i < force.rows(); ++i)
			{
				assert((t.array() * force_interpolation_weight).maxCoeff() <= 1.0 &&
					(t.array() * force_interpolation_weight).minCoeff() >= 0.0, "test function must be bounded by [0,1]");
		...
		{
			if (deformation_type_ != deformation_type_t::rotation && deformation_type_ != deformation_type_t::linear)
			{
				return;
			}
			...
			for (unsigned int i = 0; i < x_.rows(); ++i)
			{
				...
				{
					v_.block(i, 0, 1, 3) = v_.block(i, 0, 1, 3) +
